...
public class MeshLoader {
	...
	private ObjMaterialLibrary currentMatLib;
	...
	private void useMaterial(String matName) {
		ObjMaterial mat = currentMatLib.get((Object) matName);
		currentPart = new MeshPart();
		currentPart.setName(lastObjectName);
		currentPart.setMaterial(mat);
	...
	public MeshModel loadFromResource(ResourceLocation loc) throws IOException {
		...
		currentMatLib = new ObjMaterialLibrary();
		...
		while ((currentLine = lineReader.readLine()) != null) {
			...
			if (keyword.equalsIgnoreCase("g")) {
				newGroup(data);
				continue;
			}

			if (keyword.equalsIgnoreCase("mtllib")) {
				loadMaterialLibrary(loc, data);
				continue;
			}

			if (keyword.equalsIgnoreCase("usemtl")) {
				useMaterial(data);
				continue;
			}
			...
			if (keyword.equalsIgnoreCase("vn")) {
				addNormal(data);
				continue;
			}

			if (keyword.equalsIgnoreCase("vt")) {
				addTexCoord(data);
				continue;
			}
