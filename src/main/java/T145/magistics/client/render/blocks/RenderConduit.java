...
public class RenderConduit extends TileEntitySpecialRenderer<TileConduit> {
	...
	public void renderTileEntityAt(@Nonnull TileConduit conduit, double x, double y, double z, float partialTicks, int destroyStage) {
		...
		if (conduit.getDisplayQuints() > 0F) {
			BlockRenderer.renderCube(fluidSprite, mod, mod, mod, 1F - mod, mod + level, 1F - mod);
		}
		...
		for (EnumFacing facing : EnumFacing.VALUES) {
			if (conduit.isConnected(facing) && conduit.getDisplayQuints() > 0F) {
				switch (facing) {
				case UP:
					BlockRenderer.renderCube(fluidSprite, 0.5F - level / 2F, mod + level, 0.5F - level / 2F, 0.5F + level / 2F, 1F, 0.5F + level / 2F);
					break;
				case DOWN:
					BlockRenderer.renderCube(fluidSprite, 0.5F - level / 2F, 0F, 0.5F - level / 2F, 0.5F + level / 2F, mod, 0.5F + level / 2F);
					break;
				case SOUTH:
					BlockRenderer.renderCube(fluidSprite, 0.5F - level / 2F, 0F, 0.5F - level / 2F, 0.5F + level / 2F, mod, 0.5F + level / 2F);
					break;
				case NORTH:
					BlockRenderer.renderCube(fluidSprite, mod, mod, 0F, 1F - mod, mod + level, mod);
					break;
				case EAST:
					BlockRenderer.renderCube(fluidSprite, 1F - mod, mod, mod, 1F, mod + level, 1F - mod);
					break;
				case WEST:
					BlockRenderer.renderCube(fluidSprite, 0F, mod, mod, mod, mod + level, 1F - mod);
					break;
				}
			}
		}
