...
public class TilePillarSentry extends TileBase implements IInventory, ISidedInventory {
			...
			if (cooldown <= 0) {
				if (target != null && !target.isDead && target.getDistanceSq(xCoord, yCoord, zCoord) <= sentryRange && getStackInSlot(0) != null) {
					if (ammo != null) {
						...
						ISentryBehaviorItem behavior = BlockPillarSentry.getBehavior(ammo.getItem());
	...
	public boolean isItemValidForSlot(int i, ItemStack itemstack) {
		return (getStackInSlot(i) != null && getStackInSlot(i).isItemEqual(itemstack)) || BlockPillarSentry.getBehavior(itemstack.getItem()) != null;
	}
