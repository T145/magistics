...
public class MithrilWhistle extends Item {
		...
		if (!world.isRemote && (System.currentTimeMillis() - lastPlay) / 1000L > getMaxItemUseDuration(is) / 20) {
			...
			if (world.getWorldChunkManager().getBiomeGenAt(0, 0) instanceof BiomeGenEnd) {
				if (isDragonAlive(world) >= 1)
					entity.addChatMessage(new ChatComponentText(LanguageRegistry.instance().getStringLocalization("flute.dragonExist." + MithrilWhistle.rand.nextInt(4))));
			} else
				entity.addChatMessage(new ChatComponentText(LanguageRegistry.instance().getStringLocalization("flute.tips." + MithrilWhistle.rand.nextInt(3))));
		}
	...
	public ItemStack onEaten(ItemStack is, World world, EntityPlayer entity) {
		if (!world.isRemote && world.getWorldChunkManager().getBiomeGenAt(0, 0) instanceof BiomeGenEnd && isDragonAlive(world) < 1) {
			...
			entitydragon.setLocationAndAngles(0, 128, 0, MithrilWhistle.rand.nextFloat() * 360F, 0F);
