	...
	public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity)
	{
		if((entity.riddenByEntity == null) && (entity.ridingEntity == null))
		{
			if(entity instanceof EntityPlayerMP)
			{
				...
				TileElysianPortal tile = (TileElysianPortal)world.getTileEntity(x, y, z);

				if((!world.isRemote) && (tile.timebeforetp == Magistics.proxy.ticksbeforeportalteleport-1))
				{
				...
				else if(player.prevPosY == player.posY)
				{
					...
					if(tile.timebeforetp == -1) tile.timebeforetp = Magistics.proxy.ticksbeforeportalteleport;
				}
			}
		}
	}
