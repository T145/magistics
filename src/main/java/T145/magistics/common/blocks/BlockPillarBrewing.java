...
public class BlockPillarBrewing extends BlockPillarBase {
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
		...
		TilePillarBrewing pillarTile = (TilePillarBrewing) world.getTileEntity(x, y, z);
		int topSlot = 4, sideSlot = side - 2;

		if (!player.isSneaking() && hitY < 1.0F && (player.inventory.getCurrentItem() == null || !(player.inventory.getCurrentItem().getItem() instanceof ItemPotion || player.inventory.getCurrentItem().getItem() == Items.glass_bottle))) {
	...
	public TileEntity createTileEntity(World world, int meta) {
		return new TilePillarBrewing();
	}

	@Override
	@SideOnly(Side.CLIENT)
	public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
		TilePillarBrewing tile = ((TilePillarBrewing) world.getTileEntity(x, y, z));
			...
			for (int i = 0; i < 4; i++) {
				if (rand.nextInt(4) <= 1) {
					if (tile.getStackInSlot(i) != null) {
						...
						if (rotI == 3)
							subZ = -0.4F;
						int j = tile.getStackInSlot(i).getItemDamage(), k = TilePillarBrewing.getPotionResult(j, tile.getStackInSlot(4));
