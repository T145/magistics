package T145.magistics.common.lib.handlers;
...
public class PillarEventHandler {
	...
	public void onPlayerItemCrafted(PlayerEvent.ItemCraftedEvent event) {
		if (event.crafting != null
				&& event.crafting.isItemEqual(new ItemStack(
						ConfigObjects.blockBasePillar))) {
			event.player.addStat(ConfigObjects.achievementGettingStarted, 1);
		}

		if (event.crafting != null
				&& event.crafting.isItemEqual(new ItemStack(
						ConfigObjects.blockTankPillar))) {
			event.player
					.addStat(ConfigObjects.achievementCompressingLiquids, 1);
		}

		if (event.crafting != null
				&& event.crafting.isItemEqual(new ItemStack(
						ConfigObjects.blockChristmasTreeSapling))) {
	...
	public void onPlayerGetHurt(LivingHurtEvent event) {
		...
		if (event.entityLiving instanceof EntityPlayer
			...
			if (BaublesApi.getBaubles((EntityPlayer) event.entityLiving) != null) {
				ItemStack stack1 = BaublesApi.getBaubles(
						(EntityPlayer) event.entityLiving).getStackInSlot(1);
				ItemStack stack2 = BaublesApi.getBaubles(
						(EntityPlayer) event.entityLiving).getStackInSlot(2);
				...
				if (isRing1 || isRing2) {
					double d0 = event.entityLiving.posX
							+ (PillarEventHandler.rand.nextDouble() - 0.5D)
							* 64.0D;
					double d1 = event.entityLiving.posY
							+ (double) (PillarEventHandler.rand.nextInt(64) - 32);
					double d2 = event.entityLiving.posZ
							+ (PillarEventHandler.rand.nextDouble() - 0.5D)
							* 64.0D;
	 ...
	 */
	protected boolean teleportTo(Entity entity, double targetX, double targetY,
			double targetZ) {
		...
		if (entity.worldObj.blockExists(i, j, k)) {
			...
			if (flag1) {
				...
				if (entity.worldObj.getCollidingBoundingBoxes(entity,
		...
		if (!flag) {
			...
			for (int l = 0; l < short1; ++l) {
				...
				double d7 = d3 + (entity.posX - d3) * d6
						+ (PillarEventHandler.rand.nextDouble() - 0.5D)
						* (double) entity.width * 2.0D;
				double d8 = d4 + (entity.posY - d4) * d6
						+ PillarEventHandler.rand.nextDouble()
						* (double) entity.height;
				double d9 = d5 + (entity.posZ - d5) * d6
						+ (PillarEventHandler.rand.nextDouble() - 0.5D)
						* (double) entity.width * 2.0D;
				entity.worldObj.spawnParticle("portal", d7, d8, d9, (double) f,
						(double) f1, (double) f2);
			}

			entity.worldObj.playSoundEffect(d3, d4, d5, "mob.endermen.portal",
					1.0F, 1.0F);
	...
	private void setPositionEntity(Entity entity, double posX, double posY,
			double posZ) {
		...
		if (entityplayermp.playerNetServerHandler.func_147362_b()
				.isChannelOpen()) {
			EnderTeleportEvent event2 = new EnderTeleportEvent(entityplayermp,
					posX, posY, posZ, 5.0F);
			if (!MinecraftForge.EVENT_BUS.post(event2)) { // Don't indent to
															// lower patch size
				...
				((EntityLivingBase) entity).setPositionAndUpdate(
						event2.targetX, event2.targetY, event2.targetZ);
