...
import T145.magistics.client.gui.menu.ElysiumGuiEmulator;
import T145.magistics.client.gui.menu.ElysiumGuiMainMenu;
...
public class ElysiumClientHandler
{
	...
	public void guiOpen(GuiOpenEvent event) {
		if (event.gui instanceof GuiMainMenu && Magistics.proxy.customGui) {
...
//		System.out.println("Playing " + event.category + " sound: " + event.name + " class name: " + event.result.getClass().getName() + " " + event.sound.getClass().getName());
		...
		if(world != null && world.provider instanceof ElysiumWorldProvider)
		{
			if(System.currentTimeMillis() - lastPlayed >= 18000)
			{
				...
				{
					if(player.posY <= Magistics.proxy.labyrinthTop)
					{
		...
		else if(world == null && Magistics.proxy.customGui)
		{
	...
	public void onEntityRender(RenderLivingEvent.Post event)
	{
		...
		if(item != null && item.getItem() instanceof ItemElysiumStaff && item.getItemDamage() == 0)
		{
