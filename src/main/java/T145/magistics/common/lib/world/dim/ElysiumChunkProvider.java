...
public class ElysiumChunkProvider implements IChunkProvider
{
	...
	public ElysiumChunkProvider(World par1World, long par2, boolean par4)
	{
	...
	public void replaceBlocksForBiome(int chunkX, int chunkZ, Block[] blockArray, byte[] mapArray, BiomeGenBase[] paramBiomeGenBase)
	{
		...
		boolean genMazeInThisChunk = this.rand.nextInt(Magistics.proxy.mazeRoomRarity) != 0;//if normal maze
		...
		for (int z = 0; z < 16; ++z)
		{
			for (int x = 0; x < 16; ++x)
			{
				...
				for (int y = 255; y >= 0; --y)
				{
					...
					if (y < Magistics.proxy.labyrinthBottom)
					{
					...
					else
					{
						...
						//maze start
						if(y <= Magistics.proxy.labyrinthTop)
						{
							if(y < Magistics.proxy.labyrinthTop && y > Magistics.proxy.labyrinthBottom)
							{
								if(!genMazeInThisChunk)
								{
									if(x == 0 || x == 15 || z == 0 || z == 15)
									{
										...
										else
										{
											...
											if(y == Magistics.proxy.labyrinthBottom + 2)
												blockArray[index] = ConfigObjects.blockEnergyCrystal;
											else
											{
												if(this.rand.nextInt(3) < 2)
													blockArray[index] = ConfigObjects.blockQuartzBlock;
												else
													blockArray[index] = Blocks.quartz_block;
											}
										}
									}
									else
								...
								{
									blockArray[index] = null;
								} else if(y == Magistics.proxy.labyrinthBottom + 2)
								{
									blockArray[index] = ConfigObjects.blockEnergyCrystal;
								} else {
				...
				for(int x = 0; x < 5; x++)
				{
					...
					if(x < 4)
					{
						if(!maze.walls[z * 4 + x])
						{
							for(int y = 1; y <= 3; y++)
							{
								setBlock(chunkX, chunkZ, (x+1) * 3, Magistics.proxy.labyrinthBottom + y, z*3+1, blockArray, null);
								setBlock(chunkX, chunkZ, (x+1) * 3, Magistics.proxy.labyrinthBottom + y, z*3+2, blockArray, null);
							}
						}
					}
					...
					if(z < 4)
					{
						if(!maze.bottom[z * 5 + x])
						{
							for(int y = 1; y <= 3; y++)
							{
								setBlock(chunkX, chunkZ, x * 3+1, Magistics.proxy.labyrinthBottom + y, (z+1)*3, blockArray, null);
								setBlock(chunkX, chunkZ, x * 3+2, Magistics.proxy.labyrinthBottom + y, (z+1)*3, blockArray, null);
							}
						}
					}
				}
	...
	{
		setBlock(chunkX, chunkZ, 15/2, Magistics.proxy.labyrinthTop, 15/2, blockArray, Blocks.trapdoor);
		setBlock(chunkX, chunkZ, 15/2+1, Magistics.proxy.labyrinthTop, 15/2, blockArray, Blocks.trapdoor);
		setBlock(chunkX, chunkZ, 15/2, Magistics.proxy.labyrinthTop, 15/2+1, blockArray, Blocks.trapdoor);
		setBlock(chunkX, chunkZ, 15/2+1, Magistics.proxy.labyrinthTop, 15/2+1, blockArray, Blocks.trapdoor);
	}

	private void generateTreasure(int chunkX, int chunkZ, Block[] blockArray)
	{
		setBlock(chunkX, chunkZ, 15/2, Magistics.proxy.labyrinthBottom+1, 15/2, blockArray, Blocks.chest);
		setBlock(chunkX, chunkZ, 15/2+1, Magistics.proxy.labyrinthBottom+1, 15/2, blockArray, Blocks.chest);
	...
	public void populate(IChunkProvider par1IChunkProvider, int chunkX, int chunkZ)
	{
		...
		for(int i = 0; i < 16; i++)
		{
			for(int j = 0; j < 16; j++)
			{
				for(int y = Magistics.proxy.labyrinthBottom; y <= Magistics.proxy.labyrinthTop; y++)
				{
