...
public class ElysiumTeleporter extends Teleporter
{
	...
	public boolean placeInExistingPortal(Entity entity, double par2, double par4, double par6, float par8)
	{
		...
		{
			if((portals.get(i).dim == entity.dimension) && (Math.abs(portals.get(i).posX-x) < Magistics.proxy.maxportaldistance) && (Math.abs(portals.get(i).posZ-z) < Magistics.proxy.maxportaldistance))
			{
	...
	public boolean makePortal(Entity entity)
	{
		...
		while(true){
			x = MathHelper.floor_double(entity.posX) + random.nextInt(Magistics.proxy.maxportaldistance+1) - Magistics.proxy.maxportaldistance/2;
			z = MathHelper.floor_double(entity.posZ) + random.nextInt(Magistics.proxy.maxportaldistance+1) - Magistics.proxy.maxportaldistance/2;
