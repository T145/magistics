...
public class ElysiumBlockGrass extends ElysiumBlock implements IGrowable
{
	 ...
	 */
	 ...
	 {
		 return side == 1 ? this.icon_top : (side == 0 ? ConfigObjects.blockDirt.getBlockTextureFromSide(side) : this.blockIcon);
	 }

	 public Item getItemDropped(int p_149650_1_, Random p_149650_2_, int p_149650_3_)
	 {
		 return Item.getItemFromBlock(ConfigObjects.blockDirt);
	 }
	 ...
	 public void updateTick(World world, int x, int y, int z, Random rand)
	 {
		 if (!world.isRemote)
		 {
			 ...
			 {
				 world.setBlock(x, y, z, ConfigObjects.blockDirt);
			 }
			 else if (world.getBlockLightValue(x, y + 1, z) >= 9)
			 {
				 for (int l = 0; l < 4; ++l)
				 {
					 ...
					 if (world.getBlock(i1, j1, k1) == ConfigObjects.blockDirt && world.getBlockMetadata(i1, j1, k1) == 0 && world.getBlockLightValue(i1, j1 + 1, k1) >= 4 && world.getBlockLightOpacity(i1, j1 + 1, k1) <= 2)
					 {
						 world.setBlock(i1, j1, k1, ConfigObjects.blockGrass);
					 }
				 }
			 }
		 }
	 ...
	 public IIcon getIcon(IBlockAccess world, int x, int y, int z, int side)
	 {
		 ...
		 {
			 return ConfigObjects.blockDirt.getBlockTextureFromSide(side);
		 }
		 //        else
			 //        {
		 ...
		 while (l < 128)
		 {
			 ...
			 while (true)
			 {
				 if (l1 < l / 16)
				 {
					 ...
					 if (world.getBlock(i1, j1 - 1, k1) == ConfigObjects.blockGrass && !world.getBlock(i1, j1, k1).isNormalCube())
					 {
						 ++l1;
						 continue;
					 }
				 ...
				 else if (world.getBlock(i1, j1, k1).getMaterial() == Material.air)
				 {
					 if (rand.nextInt(8) != 0)
					 {
						 if (ConfigObjects.blockTallGrass.canBlockStay(world, i1, j1, k1))
						 {
							 world.setBlock(i1, j1, k1, ConfigObjects.blockTallGrass, 1, 3);
						 }
					 }
					 else
					 {
