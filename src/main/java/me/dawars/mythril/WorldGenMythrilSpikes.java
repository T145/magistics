...
public class WorldGenMythrilSpikes extends WorldGenerator {
	private Block rock;

	public WorldGenMythrilSpikes(Block p_i45464_1_) {
		rock = p_i45464_1_;
	}

	public boolean generate(World world, Random random, int x, int y, int z) {
		if (world.isAirBlock(x, y, z) && world.getBlock(x, y - 1, z) == rock) {
			int l = random.nextInt(32) + 6, i1 = random.nextInt(4) + 1;
			for (int j1 = x - i1; j1 <= x + i1; ++j1)
				for (int k1 = z - i1; k1 <= z + i1; ++k1) {
					int l2 = j1 - x;
					int i2 = k1 - z;
					if (l2 * l2 + i2 * i2 <= i1 * i1 + 1 && world.getBlock(j1, y - 1, k1) != rock) {
						return false;
					}
				}
					...
					for (int l2 = z - i1; l2 <= z + i1; ++l2) {
						...
						if (i2 * i2 + j2 * j2 <= i1 * i1 + 1)
							if (random.nextInt(10) == 0)
								world.setBlock(k1, j1, l2, MythrilMod.blockMythrilOre, 0, 2);
							else
